    ...
    def ExtractProxy(out_path:str, model:Interface):
        ...
        for key, item in model.methods.items():
            ...
            if var2 is not None:
                ...
                if include not in includes:
                        includes+=include+"\n"
            temp_s+=f"""  using {item.name}T = ara::com::proxy::MethodImpl<{var1}, {var2}>;
...
"""
        ...
        for key, item in model.broadcasts.items():
            ...
            if var1 is not None:
                ...
                if include not in includes:
                        includes+=include+"\n"
            temp_s+=f"""  using {item.name}T = ara::com::proxy::Event<{var1}>;
  {item.name}T {item.name}{"{"}"{item.name}", *this{"}"};
...
"""
        ...
        for key, item in model.attributes.items():
            ...
            if var1 is not None:
                ...
                if include not in includes:
                        includes+=include+"\n"
            temp_s+=f"""  using {item.name}T = ara::com::proxy::FieldsImpl<{var1}, {var2}>;
  {item.name}T {item.name}{"{"}"{item.name}", *this{"}"};
...
"""
        ...
        res+=f"""class {model.name}Handler final : public ara::com::ProxyHandler {"{"}\n"""
        res+=f""" public:
