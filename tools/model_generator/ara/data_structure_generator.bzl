...
ara_data_structure_generator = rule(
    ...
    attrs = {
        ...
        "tool": attr.label(
            ...
            cfg = "exec",
            allow_files = True,
            default = Label("@srp_platform//tools/model_generator/ara:data_structure_gen"),
        ),
    },
)
...
def data_structure_generator(name, model = [], visibility = []):
    ara_data_structure_generator(
        ...
        src = model,
        visibility = ["@srp_platform//visibility:private"],
    )
    native.cc_library(
        name = name,
        srcs = [":data_structure_src"],
        deps = ["@srp_platform//ara/com:com_error_domain", "@srp_platform//ara/core:Result", "@srp_platform//core/data:data_conv_lib"],
