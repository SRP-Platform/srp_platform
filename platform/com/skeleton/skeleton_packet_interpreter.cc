...
void SkeletonPacketInterpreter::ProceedFrame(
  ...
  const auto& endpoint = this->method_interpreters_list_.find(msg.method_id_);
  if (this->method_interpreters_list_.end() != endpoint) {
    endpoint->second.ProceedPacket(std::move(msg));
  }
}
bool SkeletonPacketInterpreter::RegisterMethodInterpreter(
    const uint16_t& id,
    platform::com::skeleton::interpreter::PacketInterpreter& handler_) noexcept {
  return this->method_interpreters_list_.insert({id, handler_}).second;
}
