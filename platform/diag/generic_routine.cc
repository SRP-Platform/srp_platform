 ...
 *
 */
#include "platform/diag/generic_routine.h"
...
platform::core::Result<OperationOutput> GenericRoutine::HandleMessage(
  ...
  if (requestData.size() > cRoutineIdLsbIndex) {
    ...
    return platform::diag::MakeErrorCode(
        platform ::diag::UdsDiagErrc::kInvalidMessageLengthFormat);
  }

  return platform::diag::MakeErrorCode(platform::diag::DiagErrc::kInvalidArgument, "");
}
}  // namespace diag
}  // namespace platform
