 ...
 *
 */
#include "platform/diag/monitor.h"
...
namespace platform {
namespace diag {
Monitor::Monitor(const platform::core::InstanceSpecifier& instance,
...
platform::core::Result<void> Monitor::Offer() {
  const auto model =
      platform::core::ModelDataBase::GetInstance()
          .ResolveInstanceSpecifier<platform::core::model::ModelDtc>(instance_);
  if (!model.HasValue()) {
    return model.Error();
  }
  ...
  this->offer_loop_ = std::make_unique<std::jthread>([this](auto token) {
    ...
    this->init_monitor_callback_(platform::diag::InitMonitorReason::kRestart);
  });
  return {};
}
