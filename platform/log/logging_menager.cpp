...
#include "platform/log/sink/console_log_sink.h"
...
const Logger &LoggingMenager::CreateLogger(std::string ctxId,
  ...
  std::pair<std::unordered_map<std::string, platform::log::Logger>::iterator, bool>
...
void LoggingMenager::Create(std::string appId, LogMode logMode,
  ...
  if (loger_f_ != nullptr) {
    const auto &PLATFORM_logger = loger_f_->CreateLogger("platform-", "");
    PLATFORM_logger.LogError()
