 ...
 *
 */
#include "platform/log/sink/remote_log_sink.h"
...
void RemoteLogSink::Log(const LogStream& logStream) {
  ...
  switch (logStream.GetLogLevel()) {
    case platform::log::LogLevel::kVerbose:
      log_vector.push_back(0U);
      break;
    case platform::log::LogLevel::kDebug:
      log_vector.push_back(1U);
      break;
    case platform::log::LogLevel::kInfo:
      log_vector.push_back(2U);
      break;
    case platform::log::LogLevel::kWarn:
      log_vector.push_back(3U);
      break;
    case platform::log::LogLevel::kError:
      log_vector.push_back(4U);
      break;
    case platform::log::LogLevel::kFatal:
...
void RemoteLogSink::Loop(std::stop_token token) {
  while (!token.stop_requested()) {
    ...
    if (data.has_value()) {
      if (ipc_soc.Transmit("SRP.platform.DLT", data.value()).HasValue()) {
        q.Remove();
        drop_number = 0;
      } else {
