...
#include "apps/common/logger/code/data/dlt_frame.h"
#include "apps/common/logger/code/data/dlt_log_type.h"
#include "apps/common/logger/code/data/dlt_string.h"
namespace srp {
namespace dlt {
...
int DltService::Initialize(
  ...
  if (!obj_r.has_value()) {
    platform::log::LogError() << "File not found: /opt/cpu_srp/logger_config.json";
    return -1;
  }
  auto json_obj = obj_r.value();
  {
    ...
    if (!ip_t.has_value()) {
      platform::log::LogError() << "Ip not found in config file";
      return -1;
    }
    ip_ = ip_t.value();
  }
  {
    ...
    if (!port_t.has_value()) {
      platform::log::LogError() << "tx port not found in config file";
      return -1;
    }
  ...
  }
  {
    ...
    if (!ip_t.has_value()) {
      platform::log::LogError() << "Ecu id not found in config file";
      return -1;
    }
    ec_name = ip_t.value();
  }
  platform::log::LogInfo() << "Config loaded: ip=" << ip_
...
void DltService::InitIPC() noexcept {
  if (ipc_soc.Init("SRP.platform.DLT").HasValue()) {
    ipc_soc.SetRXCallback([this](std::vector<std::uint8_t> payload) {
      if (payload.size() > 13) {
        ...
        if (!this->logs.push(std::move(r))) {
          platform::log::LogError() << "DLT msg dropped in TX queue!!";
        }
      }
    });
    ipc_soc.StartRXThread();
    platform::log::LogInfo() << "IPC socket started";
  }
}

void DltService::InitUDP() noexcept {
  if (soc.Init(ip_, "231.255.42.99", tx_port) != core::ErrorCode::kOk) {
    platform::log::LogError() << "UDP socket error";
  }
  platform::log::LogInfo() << "UDP socket started";
}
