 ...
 *
 */
#include "apps/common/someip_demon/code/connector/service_connector.h"
...
#include "apps/com/someip/message_type.h"
#include "apps/log/logging_menager.h"
#include "bindings/common/socket/stream_ipc_socket.h"
#include "core/common/condition.h"
#include "apps/common/someip_demon/code/connector/proxy_connector.h"
#include "apps/common/someip_demon/code/connector/sd_connector.h"
#include "apps/common/someip_demon/code/connector/skeleton_connector.h"

namespace srp {
namespace someip_demon {

namespace {
constexpr std::array<apps::com::someip::MessageType, 2> kSkeletonUdpType{
    apps::com::someip::MessageType::kRequest,
    apps::com::someip::MessageType::kRequestNoReturn};
constexpr std::array<apps::com::someip::MessageType, 3> kSkeletonIpcType{
    apps::com::someip::MessageType::kResponse,
    apps::com::someip::MessageType::kRequestAck,
    apps::com::someip::MessageType::kRequestNoReturnAck};
//  ---------------------------- PROXY --------------------------------------
constexpr std::array<apps::com::someip::MessageType, 2> kProxyIpcType{
    apps::com::someip::MessageType::kRequest,
    apps::com::someip::MessageType::kRequestNoReturn};
constexpr std::array<apps::com::someip::MessageType, 3> kProxyUdpType{
    apps::com::someip::MessageType::kResponse,
    apps::com::someip::MessageType::kRequestAck,
    apps::com::someip::MessageType::kRequestNoReturnAck};
...
constexpr inline bool CheckMessageType(const T& container, uint8_t type) {
  return std::any_of(
      ...
      [type](const apps::com::someip::MessageType& x) {
        return static_cast<apps::com::someip::MessageType>(type) == x;
      });
}
}  // namespace
...
ServiceConnector::ServiceConnector(const std::string& ip, const uint16_t port,
    ...
    : ip_{ip},
      port_{port},
      logger_{apps::log::LoggingMenager::GetInstance()->CreateLogger("scon")} {
  in_addr addr;
...
void ServiceConnector::UdpRxCallback(const std::string& ip,
  ...
  const auto frame_r = apps::com::someip::SomeipFrame::MakeFrame(payload);
  if (!frame_r.HasValue()) {
...
void ServiceConnector::IpcRxCallback(const uint32_t pid,
  ...
  logger_.LogDebug() << "[" << ip_ << ":" << port_ << "] New msg from: " << pid
                     << " wit payload size: "
                     << static_cast<uint16_t>(payload.size());
  const auto frame_r = apps::com::someip::SomeipFrame::MakeFrame(
