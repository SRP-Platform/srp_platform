...
void SkeletonConnector::ProcessFrame(
    uint32_t pid, apps::com::someip::SomeipFrame&& frame) noexcept {
...
void SkeletonConnector::ProcessFrame(
  ...
  if (!item.has_value()) {
    const auto res = apps::com::someip::SomeipFrame::MakeResponseFrameWithError(
        frame, apps::com::someip::MessageCode::kEUnknownService);
...
void SkeletonConnector::SendResult(const ReqDetails& det,
                                   apps::com::someip::SomeipFrame frame) {
