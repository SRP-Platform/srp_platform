...
#include "apps/com/someip/someip_sd_frame_builder.h"
...
void SDConnector::ProcessFrame(uint32_t pid,
                               apps::com::someip::SomeipFrame&& frame) noexcept {
...
void SDConnector::SdMsgHandler(const uint32_t pid,
  ...
  for (auto i = 0; i < count; i++) {
    if (*(raw.begin() + 8 + (16 * i)) == 0x01) {
      const auto t = srp::data::Convert<apps::com::someip::ServiceEntry>::Conv(
      ...
      if (t.HasValue()) {
        ...
        apps::log::LogError() << "Parssing error";
      }
      ...
      apps::log::LogError() << "unknow type";
    }
  }
}

void SDConnector::AddNewFindService(const apps::com::someip::ServiceEntry& entry,
                                    const uint32_t pid) {
...
void SDConnector::AddNewOfferService(
    const apps::com::someip::ServiceEntry& entry, const uint32_t pid) {
  const auto& s_id = entry.service_id;
  const auto& i_id = entry.instance_id;
  const uint16_t key{s_id};
...
void SDConnector::SdLoop(std::stop_token token) {
  while (!token.stop_requested()) {
    ...
    apps::log::LogError() << "[" << local_ip_ << ":" << port_
                         << "]: Loop triggered";
...
void SDConnector::SendIPCSdOffer(
  ...
  apps::com::someip::SomeipSdFrameBuilder builder{};
