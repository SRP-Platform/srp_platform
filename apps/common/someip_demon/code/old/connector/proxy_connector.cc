...
#include "platform/com/someip/someip_frame_builder.h"
...
void ProxyConnector::ProcessFrame(
    uint32_t pid, platform::com::someip::SomeipFrame&& frame) noexcept {
  ...
  if (iter == cache_.end()) {
    ...
    if (!service_det.has_value()) {
      platform::com::someip::SomeipFrameBuilder builder;
      builder.CopyFrameData(frame.header_)
          .SetMessageType(platform::com::someip::MessageType::kResponse)
          .SetReturnCode(platform::com::someip::MessageCode::kEUnknownService);
...
void ProxyConnector::ProcessFrame(
    ...
    platform::com::someip::SomeipFrame&& frame) noexcept {
...
void ProxyConnector::SendResult(const ReqDetails& det,
                                platform::com::someip::SomeipFrame frame) {
  const auto& iter = req_list_.find(frame.header_.request_id);
  if (iter == req_list_.end()) {
    return;
  }
    ...
    for (const auto& item : this->req_list_) {
      ...
      if ((std::chrono::steady_clock::now() - item.second.time_point_).count() >
        ...
        builder.SetServiceId(item.second.service_id)
            ...
            .SetReturnCode(platform::com::someip::MessageCode::kETimeout)
            .SetMessageType(platform::com::someip::MessageType::kResponse);
        auto raw = builder.BuildFrame().GetRaw();
        raw.insert(raw.begin(), 1U);
