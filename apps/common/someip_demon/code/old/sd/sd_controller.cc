...
#include "platform/com/someip/EndpointOption.h"
#include "platform/com/someip/HeaderStructure.h"
...
std::vector<db::FindServiceItem> SdController::ParseServiceList(
  ...
  std::vector<platform::com::someip::ServiceEntry> res{};
  ...
  for (auto i = 0; i < count; i++) {
    if (*(raw.begin() + 4 + (16 * i)) == 0x01) {
      const auto t =
          srp::data::Convert<platform::com::someip::ServiceEntry>::Conv(
  ...
  std::vector<platform::com::someip::EndpointOption> ip_res{};
  ...
  for (auto i = 0; i < ip_count; i++) {
    const auto t =
        srp::data::Convert<platform::com::someip::EndpointOption>::Conv(
...
void SdController::ProcessFrame(
    ...
    const platform::com::someip::SomeipFrame& frame) noexcept {
...
platform::core::Result<void> SdController::Start() noexcept {
  if (sd_loop_ != nullptr) {
    return {};
  }
...
SdController::SdController(
    ...
    : multicast_controller_{multicast_controller},
      logger_{platform::log::LoggingMenager::GetInstance()->CreateLogger(
          "sd  ", "", platform::log::LogLevel::kInfo)},
      sd_db_{sd_db} {}
...
void SdController::OfferService() noexcept {
  platform::com::someip::SomeipSdFrameBuilder builder{};
  for (const auto& item : sd_db_.GetProvideList()) {
    ...
    if (builder.GetSize() > 10) {
      ...
      builder = platform::com::someip::SomeipSdFrameBuilder{};
    }
  }
